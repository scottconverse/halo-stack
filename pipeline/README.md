...
- [ ] `prompts/emit_chunk.md` — Generate the current chunk only, writing with create/insert/str_replace so no single tool-call argument exceeds the chunk cap. On rework, the incoming report says exactly what failed - fix that, do not regenerate the artifact.
...
- [ ] `steps/verdict_route.sh` — Router: exit 0 (pass) when review.json has zero findings; exit 1 (fail) otherwise, forwarding the findings as the rework payload.
