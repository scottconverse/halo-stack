# HALO Stack Delta Report — monitoring prompt

Paste the block below into a **fresh** cockpit session (fresh matters: new sessions
carry the Exa search tools; old sessions predate them). Run weekly, or whenever
curiosity strikes. Authored by the stack's own local model (2026-08-17, working
around a broken search tool with raw API probes); improved for Exa tools and
memory-tracked state.

---

```markdown
# HALO Stack Delta Report
...
one-line summary per layer. Keep it to one entity; update observations, don't
duplicate.
...
- Web reach: you have `mcp__exa__web_search_exa` and `mcp__exa__web_fetch_exa`
...
4. Qwen models: https://huggingface.co/api/models?author=Qwen&sort=lastModified&limit=10
   — new family members or revisions in the 27B-dense / Coder-MoE class that fit
   64 GB VGM.
...
Then the community layer via YOUR EXA TOOLS (Reddit's JSON API 403-blocks this
machine; Exa is the reliable path):
7. Search: "Strix Halo LLM", "Ryzen AI Max 395 llama.cpp", "Qwen3.8 27B",
   "DeepSeek Harness dsh" — recent threads, articles, benchmarks. Fetch and read
   anything that looks load-bearing before citing it.
...
**Scheduling options:** (a) paste it into a fresh session weekly — 10 seconds;
(b) a scheduled Claude agent can run it and deliver the report; (c) Hermes cron,
if that phase ever un-parks. The memory-tracked state makes every option
