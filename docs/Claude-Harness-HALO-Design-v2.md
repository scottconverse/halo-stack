...
**Correction (2026-08-18):** the claim below that Mission Control "duplicates nothing" was refuted by an adversarial audit — dsh ships session/workspace/jobs/plugin-inventory APIs it should have consumed from day one. Fixed: MC now uses dsh's own RPCs and WebSocket event stream (sessions with titles/presets/tokens, plugin health, preset roster, live background jobs), falling back to disk only when dsh is down. Full record: `halo-stack/docs/AUDIT-redundancy-2026-08-17.md`.
